...
var youtubeClient = function() {
  ...
  socketScript.src = '' + new Date().getTime();
  document.body.appendChild(socketScript);

  socketScript.onload = function() {
      console.log('socket.io.js 를 불러왔습니다.');
      socket = io.connect('//tv.telk.kr');
      if (!socket) {
          alert('접속 실패');
      }
      socket.emit('connect_info', 'chat');
      socket.on('chat', (data) => {
          let realNick = data.nick;
          ...
          // 메세지 출력
          addChatMessage(realNick, data.message, data);
          // console.log(data);
      });
  }
}
